...
lastFrameKeys = 4
  ...
  ; DON'T USE THIS with controllers to which sending an additional
  ; strobe has a side effect, such as the Arkanoid controller or
  ; Super NES Mouse.
  .if ::DPCM_UNSAFE_PADS = 0
    jsr read_pads_once
    lda thisRead
    sta firstRead
    lda thisRead+1
